...
class ScalarsSpecificationTest {
    ...
    fun `type systems can add additional scalars with semantic meaning`() {
        ...
        val queryResponse = testedSchema.executeBlocking("{person{uuid}}")
        assertThat(queryResponse.extract<String>("data/person/uuid"), equalTo(uuid.toString()))

        val mutationResponse = testedSchema.executeBlocking(
    ...
    fun `when float is expected as an input type, both integer and float input values are accepted`() {
        ...
        val map = schema.executeBlocking("mutation{float(float: 1)}")
    ...
    fun `server can declare custom ID type`() {
        ...
        val map = testedSchema.executeBlocking("query(\$id: ID = \"$randomUUID\"){personById(id: \$id){uuid, name}}")
    ...
    fun `Schema may declare custom boolean scalar type`() {
        ...
        val value = true
        val response = schema.executeBlocking("{boolean(boolean: $value)}")
    ...
    fun `Schema may declare custom double scalar type`() {
        ...
        val value = 232.33
        val response = schema.executeBlocking("{double(double: $value)}")
    ...
    fun `Scalars within input variables`() {
        ...
        try {
            val response = schema.executeBlocking(req, values)
